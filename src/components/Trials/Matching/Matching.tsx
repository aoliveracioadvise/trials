...
export default function Matching() {
  const { pageState } = useTrialsPageContext();
    ...
    <div className="flex flex-col w-full h-full gap-2">
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="text-gray-600">{physician.medical_specialties}</p>
              </div>
              <div className="flex items-center bg-blue-50 px-3 py-1 rounded-full">
                <span className="text-blue-700 font-semibold">
                  {(() => {
                    const [matchScore, setMatchScore] = useState("Loading...");
                    ...
                    useEffect(() => {
                      const fetchMatchScore = async () => {
                        try {
                          ...
                          setMatchScore(`${data.score}% Match`);
                        } catch (error) {
                          setMatchScore("Error");
                        }
                      };
      ...
      <div className="flex w-1/2 self-center items-center mt-2 space-x-2 text-xs">
        ...
        <div className="flex w-3/5 items-center justify-center space-x-1">
          ...
          {totalPages > physicianPerPage && currentPage < totalPages - 2 && (
            <>
              <span>...</span>
              <button
